// src/components/ui/cards/FolderCard.tsx  (Corrected)
...
import StarIcon from "../assets/svg/favorite.svg";   // Filled star icon path
import StarOutlineIcon from "../assets/svg/favorite.svg"; // Outline star icon path
import SettingsIcon from "../assets/svg/threedots.svg"; // Your settings/dots icon path
// src/components/ui/cards/FolderCard.tsx (Corrected)
...
export function FolderCard({
    ...
    const folderIcons = { /* ... icon mapping ... */
    ...
    const handleButtonPress = (handler?: () => void) => (event: GestureResponderEvent) => {
    ...
    return (
        <TouchableOpacity
            ...
            activeOpacity={0.7}
        >
            {selected && <View style={[styles.selectionIndicator, { backgroundColor: colors.primary }]} />}
            <View style={styles.mainContentWrapper}>
                <View style={styles.topRow}>
                    <View style={styles.iconWrapper}>
                        {isNewFolder ? (<AddIcon width={24} height={24} fill={colors.primary} />) : (folderIcons[type])}
                    </View>
                            ...
                            {!isNewFolder && folderId && (
                                <View style={styles.actionButtons}>
                                    ...
                                    <TouchableOpacity // <-- Make the star touchable
                                        onPress={handleButtonPress(onToggleFavorite)} // <-- Call toggle favorite
                                        style={styles.actionButton}
                                        hitSlop={{ top: 10, bottom: 10, left: 10, right: 5 }}
                                    >
                                        {isFavorite ? (
                                            <StarIcon width={18} height={18} fill={colors.warning} />
                                        ) : (
                                            <StarOutlineIcon width={18} height={18} stroke={colors.secondaryText} />
                                        )}
                                    </TouchableOpacity>
                                    {/* Settings Button */}
                                    <TouchableOpacity
                                    ...
                                    >
                                        <SettingsIcon width={18} height={18} fill={colors.secondaryText} />
                                    </TouchableOpacity>
                                </View>
                            )}
                        </View>
...
const styles = StyleSheet.create({
    container: { flexDirection: "row", alignItems: "center", borderBottomWidth: StyleSheet.hairlineWidth, position: "relative", paddingHorizontal: 8, paddingVertical: 10, },
    selectionIndicator: { position: "absolute", left: 0, top: 0, bottom: 0, width: 4, borderTopLeftRadius: 2, borderBottomLeftRadius: 2, },
    mainContentWrapper: { flex: 1, flexDirection: 'column', },
    topRow: { flexDirection: "row", alignItems: "center", },
    iconWrapper: { marginRight: 12, width: 32, height: 32, justifyContent: "center", alignItems: "center", },
    detailsColumn: { flex: 1, flexDirection: 'column', },
    titleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, minHeight: 24, },
    title: { fontSize: 16, fontWeight: "500", flexShrink: 1, marginRight: 8, },
    actionButtons: { flexDirection: 'row', alignItems: 'center', },
    actionButton: { padding: 4, marginLeft: 8, justifyContent: 'center', alignItems: 'center', },
    tagsContainer: { width: '100%', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', marginTop: 4, },
});
