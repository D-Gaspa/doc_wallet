...
interface TagContextType {
    ...
    associateTag: (tagId: string, itemId: string, itemType: 'folder' | 'document', newlyCreatedTag?: Tag | null) => boolean
    disassociateTag: (tagId: string, itemId: string, itemType: 'folder' | 'document') => boolean
    getTagsForItem: (itemId: string, itemType: 'folder' | 'document') => Tag[]
    getItemsWithTag: (tagId: string, itemType?: 'folder' | 'document') => string[]
    // Tag suggestion related functions
    getFrequentlyUsedTags: (limit?: number) => Tag[]
    getRecentlyUsedTags: (limit?: number) => Tag[]
    getSuggestedTags: (itemType: 'folder' | 'document', itemName: string) => Tag[]
    // Batch operations
    batchAssociateTags: (tagIds: string[], itemIds: string[], itemType: 'folder' | 'document') => boolean
    batchDisassociateTags: (tagIds: string[], itemIds: string[], itemType: 'folder' | 'document') => boolean
}
...
export function TagProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        logger.debug("Initialized mock tags and associations",
            { tagCount: mockTags.length, associationCount: mockAssociations.length })
    }, [])
    ...
    const updateTag = (id: string, name: string, color: string): Tag | null => {
        ...
        if (tags.some(tag => tag.id !== id && tag.name.toLowerCase() === name.toLowerCase())) {
            logger.error("Cannot update tag: name already exists", { id, newName: name })
            return null
        }
    ...
    const deleteTag = (id: string): boolean => {
        ...
        setAssociations(prevAssociations => prevAssociations.filter(assoc => assoc.tagId !== id))
    ...
    const associateTag = (
        ...
        const associationExists = associations.some(
            assoc => assoc.tagId === tagId && assoc.itemId === itemId && assoc.itemType === itemType
        )

        if (associationExists) {
            logger.debug("Tag association already exists", { tagId, itemId, itemType })
            return false
        }
    ...
    const disassociateTag = (tagId: string, itemId: string, itemType: 'folder' | 'document'): boolean => {
        const associationExists = associations.some(
            assoc => assoc.tagId === tagId && assoc.itemId === itemId && assoc.itemType === itemType
        )

        if (!associationExists) {
            logger.error("Tag association not found for removal", { tagId, itemId, itemType })
            return false
        }
    ...
    // Query operations
    const getTagsForItem = (itemId: string, itemType: 'folder' | 'document'): Tag[] => {
        const tagIds = associations
            .filter(assoc => assoc.itemId === itemId && assoc.itemType === itemType)
            .map(assoc => assoc.tagId)

        return tags.filter(tag => tagIds.includes(tag.id))
    }

    const getItemsWithTag = (tagId: string, itemType?: 'folder' | 'document'): string[] => {
        return associations
            .filter(assoc =>
                assoc.tagId === tagId &&
    ...
    const getSuggestedTags = (itemType: 'folder' | 'document', itemName: string): Tag[] => {
        const nameLower = itemName.toLowerCase()

        // Simple keyword matching
        const matchedTags = tags.filter(tag => {
            // If the item name contains the tag name or vice versa
            return nameLower.includes(tag.name.toLowerCase()) ||
        ...
        const frequentTagsForType = getFrequentlyUsedTags(3).filter(tag => {
            ...
            return associations.some(assoc => assoc.tagId === tag.id && assoc.itemType === itemType)
        })
    ...
    const batchAssociateTags = (
        ...
        const allTagsExist = tagIds.every(tagId => tags.some(tag => tag.id === tagId))
        if (!allTagsExist) {
